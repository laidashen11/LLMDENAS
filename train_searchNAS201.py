...
from llm_guidanceNAS201 import llm_guided_search

parser = argparse.ArgumentParser("NAS201")
parser.add_argument('--data', type=str, default='/fastdatasets/ImageNet16', help='location of the data corpus')
...
parser.add_argument('--api_path', type=str, default='/fastdatasets/NAS-Bench-102-v1_0-e61699.pth',
...
def train(model, train_queue, criterion, optimizer, gen, pop_size=args.pop_size):
    ...
    for step, (inputs, targets) in enumerate(train_queue):
        ...
        model.update_alphas(population.get_population()[step % pop_size].arch_parameters[0])
        ...
        population.get_population()[step % pop_size].objs.update(loss.data.cpu().item(), n)
        population.get_population()[step % pop_size].top1.update(prec1.data.cpu().item(), n)
        population.get_population()[step % pop_size].top5.update(prec5.data.cpu().item(), n)
            ...
            logging.info(
                "Using Training batch #{} for {}/{} architecture with loss: {}, prec1: {}, prec5: {}".format(step,
                                                                                                             ...
                                                                                                             population.get_population()[
                                                                                                                 step % pop_size].objs.avg,
                                                                                                             population.get_population()[
                                                                                                                 step % pop_size].top1.avg,
                                                                                                             population.get_population()[
                                                                                                                 step % pop_size].top5.avg))
        # break


def validation(model, valid_queue, criterion, gen, pop_size=args.pop_size):
    model.eval()
    ...
    for i in range(len(population.get_population())):
        model.update_alphas(population.get_population()[i].arch_parameters[0])
        assert model.check_alphas(population.get_population()[i].arch_parameters[0])
        ...
        population.get_population()[i].set_truetop1(float(valid_acc))
...
if __name__ == '__main__':
    ...
    if args.cutout:
        train_data, valid_data, xshape, num_classes = get_datasets(name=args.dataset, root=args.data,
                                                                   cutout=args.cutout)
    else:
        train_data, valid_data, xshape, num_classes = get_datasets(name=args.dataset, root=args.data, cutout=-1)
    logging.info(
        "train data len: {}, valid data len: {}, xshape: {}, #classes: {}".format(len(train_data), len(valid_data),
                                                                                  xshape, num_classes))
    ...
    arch_index = api.query_index_by_arch(model.genotype())
    print("=" * 60)
    print(f'arch_str: {arch_str}, arch_index: {arch_index}')
    print("=" * 60)
    if args.dataset == 'cifar10':
    ...
    for epoch in range(args.epochs):
        ...
        train(model, train_queue, criterion, optimizer, epoch + 1, current_pop_size)
        ...
        validation(model, valid_queue, criterion, epoch + 1, current_pop_size)
        ...
        if args.dataset == 'cifar10':
            ...
            valid_acc = get_arch_score(api, arch_index, 'cifar10-valid', 200, val_acc_type)
            print("=" * 60)
            print(f'arch_str: {arch_str}, arch_index: {arch_index}, test_acc: {test_acc}, valid_acc: {valid_acc}')
            print("=" * 60)
        ...
        #         # LLM Guided Search
        #         pop = llm_guided_search(model, population, device, epoch + 1, checkpoint_list)
        ...
        pop = de.evolve(population, current_pop_size)
        population = pop
