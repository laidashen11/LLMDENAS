...
def plot_cell_from_alphas(alphas, filename):
	g = Digraph(
		format='pdf',
		edge_attr=dict(fontsize='20', fontname="Times-Roman"),
		node_attr=dict(style='filled', shape='rect', align='center', fontsize='20', height='0.5', width='0.5', penwidth='2', fontname="Times-Roman"),
		engine='dot')
	...
	num = alphas.size()[0]
	ops = utils.derive_architecture(alphas)
	#print("\nops: {}\n\n".format(ops))
	...
	g.render(filename, view=True, cleanup=True)
...
def plot(genotype, filename, view = True):
	...
	g.render(filename, view = view, cleanup=True)
