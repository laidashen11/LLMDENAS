...
from llm_guidance import llm_guided_search
...
def train(model, train_queue, criterion, optimizer, gen, pop_size=args.pop_size):
	model.train()
	for step, (inputs, targets) in enumerate(train_queue):
		# discrete_alpha = utils.discretize(population.get_population()[step % pop_size].get_arch_parameters(), device)
		model.copy_arch_parameters(population.get_population()[step % pop_size].get_arch_parameters())
		...
		prec1, prec5 = utils.accuracy(logits, targets, topk = (1, 5))
		population.get_population()[step % pop_size].objs.update(loss.data, n)
		population.get_population()[step % pop_size].top1.update(prec1.data, n)
		population.get_population()[step % pop_size].top5.update(prec5.data, n)

		if (step + 1) % 100 == 0:
			logging.info("[{} Generation]".format(gen))
			logging.info("Using Training batch #{} for {}/{} architecture with loss: {}, prec1: {}, prec5: {}".format(step, step % pop_size,
																							len(population.get_population()),
																							population.get_population()[step % pop_size].objs.avg,
																							population.get_population()[step % pop_size].top1.avg,
																							population.get_population()[step % pop_size].top5.avg))

def validation(model, valid_queue, criterion, gen, population, pop_size=args.pop_size):
	model.eval()
	# if gen > 1:
	# 	for i in range(pop_size):
	# 		valid_start = time.time()
	# 		population.get_population()[i+pop_size].objs.reset()
	# 		population.get_population()[i+pop_size].top1.reset()
	# 		population.get_population()[i+pop_size].top5.reset()
	#
	# if gen == 1:
	for i in range(len(population.get_population())):
		valid_start = time.time()
	...
	with torch.no_grad():
		for step, (inputs, targets) in enumerate(valid_queue):
			discrete_alphas = utils.discretize(population.get_population()[step % population.get_population_size()].arch_parameters, device)
			model.copy_arch_parameters(discrete_alphas)
			n = inputs.size(0)
			inputs = inputs.to(device)
			targets = targets.to(device)
	...
	if gen == 1:
		...
		for i in range(pop_size):
			...
			logging.info("[{} Generation] {}/{} finished with validation loss: {}, prec1: {}, prec5: {}, scaling factor: {}".format(gen, i+1, len(population.get_population()),
																												population.get_population()[i].objs.avg,
																												population.get_population()[i].top1.avg,
																												population.get_population()[i].top5.avg,
	...
	if gen > 1:
		...
		for i in range(pop_size):
			...
			if population.get_population()[i].top1.avg > population.get_population()[i+pop_size].top1.avg:
				pop_index.append(i+pop_size)
			else:
				pop_index.append(i)
		pop_index.sort()
		...
		for i in range(pop_size):
			...
			logging.info('genotype = %s', genotype)
			logging.info(
				"[{} Generation] {}/{} finished with validation loss: {}, prec1: {}, prec5: {}, scaling factor: {}".format(gen, i + 1, len(population.get_population()),
																									    population.get_population()[i].objs.avg,
																									    population.get_population()[i].top1.avg,
																									    population.get_population()[i].top5.avg,
																										population.get_population()[i].get_mutate_factor()))
...
current_pop_size = args.pop_size
# The archive is used to store the hypernetwork and population information of each generation.
checkpoint_list = []
# main for loop
for epoch in range(args.epochs):
	...
	train(model, train_queue, criterion, optimizer, epoch + 1, pop_size=current_pop_size)
	...
	validation(model, valid_queue, criterion, epoch + 1, population, pop_size=current_pop_size)
	population.pop_sort()
	# Keep the first args.pop_size best individuals
	if population.get_population_size() > args.pop_size:
		population.get_population()[:] = population.get_population()[:args.pop_size]
	...
	# Determining whether the threshold has been reached requires a rollback.
	# if len(checkpoint_list) >= 3:
	# 	last_top1 = checkpoint_list[-1]['population'].get_population()[0].top1.avg
	# 	second_last_top1 = checkpoint_list[-2]['population'].get_population()[0].top1.avg
	# 	third_last_top1 = checkpoint_list[-3]['population'].get_population()[0].top1.avg
		
	# 	if last_top1 < third_last_top1 and second_last_top1 < third_last_top1:
	# 		model.load_state_dict(checkpoint_list[-3]['model'])
	# 		population = checkpoint_list[-3]['population']
	# 		LLM generates new architecture distribution
	# 		population = llm_guided_search(model, population, train_queue, valid_queue, criterion, device, args, epoch + 1, checkpoint_list)

	# LLM generates new architecture distribution
	population = llm_guided_search(model, population, train_queue, valid_queue, criterion, device, args, epoch + 1, checkpoint_list)
	current_pop_size = population.get_population_size()
	# Mutation operation
	pop = ga.evolve(population, epoch, args.epochs, pop_size=current_pop_size)
	population = pop
